...
class StatsListener():
    def __init__(self):
        if not bool(int(minqlxtism.get_cvar("zmq_stats_enable"))):
            self.done = True
            return

        stats = minqlxtism.get_cvar("zmq_stats_ip")
        port = minqlxtism.get_cvar("zmq_stats_port")
        if not port:
            port = minqlxtism.get_cvar("net_port")
        self.address = "tcp://{}:{}".format("127.0.0.1" if not stats else stats, port)
        self.password = minqlxtism.get_cvar("zmq_stats_password")
    ...
    def keep_receiving(self):
        ...
        try:
            ...
            while True: # Will throw an expcetion if no more data to get.
                ...
                minqlxtism.EVENT_DISPATCHERS["stats"].dispatch(stats)

                if stats["TYPE"] == "MATCH_STARTED":
                    self._in_progress = True
                    minqlxtism.EVENT_DISPATCHERS["game_start"].dispatch(stats["DATA"])
                elif stats["TYPE"] == "ROUND_OVER":
                    minqlxtism.EVENT_DISPATCHERS["round_end"].dispatch(stats["DATA"])
                    ...
                    if self._in_progress:
                        minqlxtism.EVENT_DISPATCHERS["game_end"].dispatch(stats["DATA"])
                    self._in_progress = False
                elif stats["TYPE"] == "PLAYER_DEATH":
                    # Dead player.
                    sid = int(stats["DATA"]["VICTIM"]["STEAM_ID"])
                    if sid:
                        player = minqlxtism.Plugin.player(sid)
                    else: # It's a bot. Forced to use name as an identifier.
                        player = minqlxtism.Plugin.player(stats["DATA"]["VICTIM"]["NAME"])
                    ...
                    if not stats["DATA"]["KILLER"]:
                        ...
                        if sid_killer:
                            player_killer = minqlxtism.Plugin.player(sid_killer)
                        else: # It's a bot. Forced to use name as an identifier.
                            player_killer = minqlxtism.Plugin.player(stats["DATA"]["KILLER"]["NAME"])

                    minqlxtism.EVENT_DISPATCHERS["death"].dispatch(player, player_killer, stats["DATA"])
                    if player_killer:
                        minqlxtism.EVENT_DISPATCHERS["kill"].dispatch(player, player_killer, stats["DATA"])
                    ...
                    if sid:
                        ...
                        player = minqlxtism.Plugin.player(stats["DATA"]["KILLER"]["NAME"])
                    old_team = stats["DATA"]["KILLER"]["OLD_TEAM"].lower()
                    new_team = stats["DATA"]["KILLER"]["TEAM"].lower()
                    if old_team != new_team:
                        res = minqlxtism.EVENT_DISPATCHERS["team_switch"].dispatch(player, old_team, new_team)
                        if res is False:
                            player.put(old_team)
