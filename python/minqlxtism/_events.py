...
class EventDispatcher:
    ...
    def dispatch(self, *args, **kwargs):
        ...
        Handlers have several options for return values that can affect the flow:
            - minqlxtism.RET_NONE or None -- Continue execution normally.
            - minqlxtism.RET_STOP -- Stop any further handlers from being called.
            - minqlxtism.RET_STOP_EVENT -- Let handlers process it, but stop the event
                at the engine-level.
            - minqlxtism.RET_STOP_ALL -- Stop handlers **and** the event.
            ...
            for plugin in plugins:
                for handler in plugins[plugin][i]:
                    try:
                        ...
                        if res == minqlxtism.RET_NONE or res is None:
                            continue
                        elif res == minqlxtism.RET_STOP:
                            return True
                        ...
                        minqlxtism.log_exception(plugin)
                        continue
    ...
    def add_hook(self, plugin, handler, priority=minqlxtism.PRI_NORMAL):
        ...
        :type priority: minqlxtism.PRI_LOWEST, minqlxtism.PRI_LOW, minqlxtism.PRI_NORMAL, minqlxtism.PRI_HIGH or minqlxtism.PRI_HIGHEST
        :raises: ValueError

        """
        if not (minqlxtism.PRI_HIGHEST <= priority <= minqlxtism.PRI_LOWEST):
            raise ValueError("'{}' is an invalid priority level.".format(priority))

        if self.need_zmq_stats_enabled and not bool(int(minqlxtism.get_cvar("zmq_stats_enable"))):
    ...
    def remove_hook(self, plugin, handler, priority=minqlxtism.PRI_NORMAL):
...
class ConsolePrintDispatcher(EventDispatcher):
    ...
    those with :func:`minqlxtism.console_print`.
...
class ClientCommandDispatcher(EventDispatcher):
    ...
    def dispatch(self, player, cmd):
        ...
        ret = minqlxtism.COMMANDS.handle_input(player, cmd, minqlxtism.ClientCommandChannel(player))
        if ret is False:
            return False
...
class ServerCommandDispatcher(EventDispatcher):
    ...
    including :func:`minqlxtism.send_server_command`. Can be cancelled.
...
class SetConfigstringDispatcher(EventDispatcher):
    ...
    stop this event and use :func:`minqlxtism.set_configstring` to modify it, but a
...
class ChatEventDispatcher(EventDispatcher):
    ...
    def dispatch(self, player, msg, channel):
        ret = minqlxtism.COMMANDS.handle_input(player, msg, channel)
...
class VoteEndedDispatcher(EventDispatcher):
    ...
    def dispatch(self, passed):
        ...
        cs = minqlxtism.get_configstring(9)
        if not cs:
            minqlxtism.get_logger().warning("vote_ended went off without configstring 9.")
            return

        res = _re_vote.match(cs)
        vote = res.group("cmd")
        args = res.group("args") if res.group("args") else ""
        votes = (int(minqlxtism.get_configstring(10)), int(minqlxtism.get_configstring(11)))
